...
class _SleepStoriesScreenState extends State<SleepStoriesScreen> {
  ...
  Widget _buildFeaturedStory() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final featuredStory = snapshot.data!.docs.first;
        final data = featuredStory.data() as Map<String, dynamic>;
  ...
  Widget _buildFeaturedCard(Map<String, dynamic> data) {
    final String title = data['title']?.toString() ?? 'Untitled Story';
    ...
    return Expanded(
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(
              child: Text('No sleep stories available'),
            );
          }
  ...
  void _openStory(Map<String, dynamic> storyData) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AudioPlayerScreen(
          ...
          imageUrl: storyData['imageUrl'] ?? 'https://via.placeholder.com/400x200',
        ),
      ),
    );
  }
