...
class _BreathingExerciseScreenState extends State<BreathingExerciseScreen> {
  ...
  Future<void> _checkFirestoreData() async {
    try {
      ...
      final snapshot = await _firestore.collection('breathing_exercise').get();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            _buildAppBar(),
            _buildFeaturedBreathing(),
            const SizedBox(height: 24),
            _buildAllBreathingTitle(),
            _buildBreathingList(),
          ],
        ),
      ),
  ...
  Widget _buildFeaturedBreathing() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final featuredBreathing = snapshot.data!.docs.first;
        final data = featuredBreathing.data() as Map<String, dynamic>;
  ...
  Widget _buildFeaturedCard(Map<String, dynamic> data) {
    final String title = data['title']?.toString() ?? 'Untitled Exercise';
  ...
  Widget _buildAllBreathingTitle() {
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              final breathing = snapshot.data!.docs[index];
              final data = breathing.data() as Map<String, dynamic>;
  ...
  void _openBreathing(Map<String, dynamic> breathingData) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AudioPlayerScreen(
          title: breathingData['title'] ?? 'Untitled',
          description: breathingData['description'] ?? '',
          audioUrl: breathingData['audioUrl'] ?? '',
          imageUrl: breathingData['imageUrl'] ?? 'https://via.placeholder.com/400x200',
        ),
      ),
    );
  }
